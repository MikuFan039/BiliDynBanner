...
export default {
  ...
  async init(container, taperLine, extConfig, resolveUrl, activeUrl) {
    ...
    // 游戏 JS 内部通过 document.querySelector(".bili-header__banner") 查找 container
    ...
    // ── 修复游戏 JS 中的 MIME 类型拼写错误 ──────────────────────────────────
    // 游戏源码中叉号图片的 src 写成了 "data:img/png;base64,..."
    ...
    // 游戏 JS 把 closeIcon/closeButton 直接 append 到 container（.bili-header__banner）
    ...
    clickZone.addEventListener("click", (clickEvent) => {
      ...
      // 阶段二：展开前插入 <base>，使扩展运行期间的资源请求路径正确解析
      runtimeBase = insertBase(baseUrl);

      // 过渡动画结束后才触发扩展进入全屏模式，并注册退出回调
      window.clearTimeout(transitionTimer);
      transitionTimer = window.setTimeout(() => {
        extInstance.toggleExtend?.(true);

        // onExit 在 toggleExtend 之后注册，确保扩展已进入运行状态
        extInstance.onExit = () => {
          isExpanded = false;
          container.dispatchEvent(
          ...
          // 阶段二：扩展退出时移除 <base>，断开 MIME 修复 observer
          runtimeBase?.remove();
          runtimeBase = null;
