 ...
 *   阶段二（扩展运行）：脚本加载完成后重新插入，exit() 时移除
 *   两次进入高级扩展之间 <base> 不存在，对页面无影响
 */
...
export default {
  ...
  async init(container, taperLine, extConfig, resolveUrl, activeUrl) {
    ...
    let active = false;
    let exitTimer = NaN;
    let advExtBaseEl = null; // 阶段二的 <base>，enter 时创建，exit 时销毁
    ...
    clickZone.addEventListener("click", async (e) => {
      ...
      // 阶段二：高级扩展启动前重新插入 <base>，使运行期间的资源请求路径正确
      advExtBaseEl = insertBase(baseUrl);
