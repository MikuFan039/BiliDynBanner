...
**此处是动态 Banner 的高级/游戏扩展，参照官方前端的实现**
...
启用高级/游戏扩展需保证 banner 的 json 信息中包含了`extensions`对象，例：
...
运行`summer2022`和`autumn2022`高级扩展时会使用`detect-gpu`检测 GPU 等级，也可以通过配置`extensions`跳过
